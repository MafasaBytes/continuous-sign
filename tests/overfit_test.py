...
Tests if the architecture can memorize a small dataset (3-5 samples)
...
def train_overfit_test(
    ...
    """
    print(f"\n{'='*70}")
    print(f"OVERFITTING TEST - MobileNetV3 Sign Language Model")
    print(f"{'='*70}")
...
def plot_results(history: Dict, save_path: Path = None):
    ...
    axes[1].set_title('Word Error Rate', fontsize=14, fontweight='bold')
...
def main():
    ...
    num_samples = 10
    num_epochs = 3000  # Push for < 5% WER target
    learning_rate = 0.0005  # Further reduced for final convergence
    ...
    try:
        ...
        # Plot results
        plot_path = Path("overfit_test_results.png")
        plot_results(history, save_path=plot_path)
        
        # Save detailed report
        report_path = Path("overfit_test_report.txt")
        with open(report_path, 'w') as f:
            f.write("="*70 + "\n")
            f.write("OVERFITTING TEST REPORT - MobileNetV3 Sign Language Model\n")
            f.write("="*70 + "\n\n")
        ...
        if results['overall_passed']:
            print("\nSUCCESS! The model can learn and memorize the training data.")
            print("The architecture is capable of learning the vocabulary and reducing WER.")
        else:
            print("\nWARNING! The model failed to overfit to the training data.")
