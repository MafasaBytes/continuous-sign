...
def ctc_greedy_decode(log_probs: torch.Tensor, blank_idx: int = 0) -> List[List[int]]:
    ...
    # Check if we need to transpose: if middle dim is smaller, likely [N, T, C]
    T, N, C = log_probs.shape
    if N > T:  # Likely [N, T, C], transpose to [T, N, C]
...
def ctc_beam_search_decode(log_probs: torch.Tensor, blank_idx: int = 0, 
                          beam_width: int = 10) -> List[List[int]]:
    """
    Beam search CTC decoding with proper prefix beam search algorithm (GPU-optimized).
